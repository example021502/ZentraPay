...
// ============================================
router.post(
  "/paystack/bank_transfer/initialize",
  authenticateToken,
  async (req, res) => {
    ...
    try {
      ...
      const params = JSON.stringify({
        type: type || "nuban",
        name: account_name || "External Recipient",
        account_number: account_number,
        bank_code: bank_code,
        currency: currency_code,
        description: description ?? "External bank payment transfer",
      });
      ...
      recipientResponse.write(params);
      recipientResponse.end();

      if (!recipientResponse.status) {
        return res.status(400).json({
          success: false,
          message:
            recipientResponse.message ?? "Error creating payment recipient!",
        });
      }
      ...
      // Fixed: referencing properties matching the RETURNING clause definition
      ...
      console.error(
        "Initiate External Payment Error:",
...
router.get("/external/verify", async (req, res) => {
  try {
    ...
    // Fixed: column lookup targeting payment_reference
    const transactionResult = await pool.query(
    ...
    // Fixed: aligning variables with correct database column targets
    ...
    if (walletResult.rowCount === 0) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to update wallet balance" });
    }
