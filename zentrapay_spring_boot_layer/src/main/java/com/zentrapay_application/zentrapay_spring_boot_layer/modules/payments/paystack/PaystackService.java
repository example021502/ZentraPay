...
public class PaystackService implements PaymentGatewayService {
    ...
    /**
     * Initializes a Paystack collection (charge) transaction — the "fund my wallet"
     * flow, distinct from {@link #processDisbursement}'s payout flow.
     * <p>
     * API Endpoint: POST https://api.paystack.co/transaction/initialize
     * Authentication: Bearer {secretKey}
     *
     * @param email    the paying customer's email (required by Paystack)
     * @param amount   the amount to charge, in major currency units (e.g. GHS, not pesewas)
     * @param currency the ISO currency code
     * @param reference a unique reference the caller already persisted a PENDING transaction under
     * @return the Paystack access code / authorization URL needed to complete payment client-side
     */
    public PaystackAccessCodeResponseDTO initializeTransaction(String email, BigDecimal amount, String currency, String reference) {
        int amountInSmallestUnit = convertToSmallestUnit(amount, currency);

        Map<String, Object> payload = new HashMap<>();
        payload.put("email", email);
        payload.put("amount", amountInSmallestUnit);
        ...
        log.info("[PAYSTACK_SERVICE] Initializing transaction: ref={}, amount={} {}", reference, amount, currency);
        ...
        if (body == null || !Boolean.TRUE.equals(body.get("status"))) {
            ...
            log.error("[PAYSTACK_SERVICE] Failed to initialize transaction: {}", errorMsg);
            throw new RuntimeException("Failed to initialize Paystack transaction: " + errorMsg);
        }

        Map<String, Object> data = (Map<String, Object>) body.get("data");
        return new PaystackAccessCodeResponseDTO(
    ...
    /**
     * Verifies the {@code x-paystack-signature} header on an incoming webhook call:
     * HMAC-SHA512 of the raw request body, keyed with the Paystack secret key,
     * hex-encoded. See https://paystack.com/docs/payments/webhooks/#verify-events.
     */
    public boolean verifyWebhookSignature(String rawBody, String signatureHeader) {
        ...
        try {
            ...
            log.error("[PAYSTACK_SERVICE] Webhook signature verification failed: {}", e.getMessage());
            return false;
        }
    }
